    ...
    setupExpandables: function(){
      ...
      .on('click', self.selectors.expanders, function(event){
        ...
        if(toggleElem.is(':visible')){
          ...
          $.cookie('dropdown', 'close-it', { expires: 7 });
        }else{
          toggler.addClass('opened');
          console.log(toggleElem.is(':visible'));
          $.cookie('dropdown', 'keep-open', { expires: 7 });
        }
        toggleElem.slideToggle(125);
      });

      var cookie_check = $.cookie('dropdown');
      if (cookie_check == 'close-it') {
          console.log('cookie!');
          $(self.selectors.expandables).hide();
          $('#filament #additional-info h3.expander').removeClass('opened');
      }else{
        $(self.selectors.expandables).show();
          $('#filament #additional-info h3.expander').addClass('opened');
      }
